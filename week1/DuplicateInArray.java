...
public class DuplicateInArray {
	//Return repeated number in unsorted array
    ...
    //Return repeated number and missing number in unsorted array
    public ArrayList<Integer> repeatedNumberWithMissing(final List<Integer> a) {
    ...
    //Return repeated number and missing number in unsorted array
    public ArrayList<Integer> repeatedNumberWithMissingEquation(final List<Integer> a) {
        ArrayList<Integer> result = new ArrayList<>();
        ...
        for (long i = 0; i < a.size(); i++ ) {
            X = X.add(BigInteger.valueOf(a.get((int)i)));

            Y = Y.add(BigInteger.valueOf(a.get((int)i) * a.get((int)i)));
        }

        for (long i = 0; i < a.size(); i++ ) {
            X = X.subtract(BigInteger.valueOf((int)i));

            Y = Y.subtract(BigInteger.valueOf((int)i * (int)i));
        }

        int x = 0;
        int y = 0;

        BigInteger z = (Y.divide(X)); //(A + B)

        BigInteger A = X.add(z);

        int repeated = A.intValue() / 2;
        int missing = repeated - X.intValue();
    ...
    //Return repeated number and missing number in unsorted array
    public ArrayList<Integer> repeatedNumberWithMissingEquation2(final List<Integer> a) {
        ArrayList<Integer> result = new ArrayList<>();

        long X = 0; //(A + B) sum
        long Y = 0; // (A - B)(A + B) sum of square

        for (long i = 0; i < a.size(); i++ ) {
            X += a.get((int)i);
            X -= (int)i;
            Y += a.get((int)i) * a.get((int)i);
            Y -= (int)i * (int)i;
        }
        X -= a.size();
        Y -= a.size() * a.size();
    ...
    //Return repeated number and missing number in unsorted array
    public ArrayList<Integer> repeatedNumberWithMissingSorting(final List<Integer> a) {
        ArrayList<Integer> result = new ArrayList<>();
        Collections.sort(a);
    ...
    //Return repeated number and missing number in unsorted array
    public ArrayList<Integer> repeatedNumberWithMissingSorting2(final List<Integer> a) {
        ArrayList<Integer> result = new ArrayList<>();
        Collections.sort(a);
        int n = a.size();
        int repeated = -1;
        ...
        sum += a.get(a.size() - 1).intValue();

        missing = (int) ((1L * n * (1L * n + 1)) / 2 - sum + repeated);
